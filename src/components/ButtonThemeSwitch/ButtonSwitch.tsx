...
export const ButtonThemeSwitch: any = () => {
    const ctxTheme = useTheme();

    const handleThemeToggle = () => {
        if (ctxTheme) {
            ctxTheme.setTheme(ctxTheme.theme === "dark" ? "light" : "dark");
        }
    };
    ...
    return (
       ...
       c-6.337-2.108-11.473-6.557-14.463-12.528C11.899,15.541,11.11,10.16,12.002,4.936z"
        ...
        //     className={` w-96  mx-auto fixed top-0 right-0 left-0 text-center p-2 m-2 rounded-md font-semibold cursor-pointer
        //     ${ctxTheme?.theme === "dark" ? "bg-white text-black" : "bg-black text-white"}`}
        // >
        //     {ctxTheme?.theme === "dark" && "Mudar para Light"}
        //     {ctxTheme?.theme === "light" && "Mudar para Dark"}
        // </button>
    );
};
